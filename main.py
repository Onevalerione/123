...
class Virus(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, img = 'Virus.png'):
        super().__init__()
        '''выгружаем изображение вируса и устаналиваем несколько на указанные позиции'''
        self.image = pygame.image.load(img).convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.x = x
        self.rect.y = y
        '''устанавливаем зону действия вируса по оси x'''
        self.start = x
        self.stop = x + random.randint(80, 90)
        #self.start = y
    ...
    def update(self):
        ...
        if self.rect.x >= self.stop:
            self.rect.x = self.stop
            self.direction = -1
        '''если начальное положение вируса левее его допустимой зоны действия по оси x,
        то начальное положение меняется на самую левую зону зону действия'''
        if self.rect.x <= self.start:
            self.rect.x = self.start
            self.direction = 1
